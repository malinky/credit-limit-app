...
class AWSCognitoWrapper
{
    ...
    public function __construct()
    {
        ...
        if($env == "local") {
            ...
            $this->credentials_key = getenv('credentials_key');
            $this->credentials_secret = getenv('credentials_secret');
         }
         else {
             // we're running on aws, get settings from server variables
            $this->region = $_SERVER['region'];
            $this->client_id = $_SERVER['client_id'];
            $this->userpool_id = $_SERVER['userpool_id'];
         }
    }
    ...
    public function initialize() : void
    {
        $env = getenv('environment');
        if($env == "local") {
            /// running on local, must include the credentials
            $this->client = new CognitoIdentityProviderClient([
                'version' => '2016-04-18',
                'region' => $this->region,
                'credentials' => [
                    'key'    => $this->credentials_key,
                    'secret' => $this->credentials_secret,
                ],
            ]);
        }
        else {
